...
describe("bankedExtraDice turn ownership", () => {
  it("Invariant K: auto-pass while resolving banked dice must NOT advance nextActorId when banked dice remain", () => {
    ...
    const session: SessionState = {
      ...
      // Canonical storage is bankedExtraDice (legacy bankedExtraRolls is inbound-only).
      bankedExtraDice: 2,
      pendingDice: undefined,
    };

    // When bankedExtraDice > 0, the next roll must roll exactly that many dice.
    ...
    expect(res.nextState.bankedExtraDice ?? 0).toBe(1);
