...
describe("bankedExtraDice turn ownership", () => {
  it("Invariant K: auto-pass while resolving banked dice must NOT advance nextActorId when banked rolls remain", () => {
    const p0 = P("p0");
    const p1 = P("p1");
    ...
    // Rolling a 1 earns an extra roll, so the bank should end at 1 and the turn must stay with p0.
    ...
    expect(res.nextState.bankedExtraRolls ?? 0).toBe(1);
